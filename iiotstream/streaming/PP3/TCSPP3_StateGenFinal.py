...
try:
    pp3_vibr_file = pd.read_csv(pp3file, usecols = ['time','ax','ay','az'])
    raw_pp_current=pd.read_csv('PP3_EM11.csv', usecols=['time', 'c1', 'c2', 'c3'])
...
#merging boards using proximity sensor fusion using PP3 entry proximity triggers
PROC_ONE_END_FLAG = 0
...
# print('PP3 working time mode: ', PP_events[PP_events.event==1].working_time.mode().mean())
# parameters=open('/home/richard/Desktop/LiveParameters/PP3parameters.txt','a+')
# parameters.write(timestr +'\tPP3 working time mode: ' + str(PP_events[PP_events.event==1].working_time.mode().mean())+'\n')
# parameters.close()


####If you want to save PP3 states/events to a file (Also need to uncomment PP_events portion above to create new DF) ######

# if not os.path.exists('/mnt/UltraHD/streamingStates/PP3'):

#     os.makedirs('/mnt/UltraHD/streamingStates/PP3')

# directory='/mnt/UltraHD/streamingStates/PP3'

# timestr = time.strftime("%Y-%m-%d_%H-%M-%S")
# print("Saving States with file name -", timestr+"_PP3.csv")
# PP_events.to_csv('/mnt/UltraHD/streamingStates/PP3/'+ timestr+"_PP3.csv")
# pcb_merge_pp3.to_csv('/mnt/UltraHD/streamingStates/merging/PP3/'+ timestr+"_merging_correction_PP3.csv")