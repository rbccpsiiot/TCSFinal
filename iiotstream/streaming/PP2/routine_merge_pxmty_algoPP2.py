    ...
    while (END_FLAG == 0):
        ...
        if (i >= len(inp_df)):                     #already reached end of dataframe
            ...
            if (inp_df.weightage[i] <= THRESHOLD):
                str_tmstmp = str(pd.Timestamp(inp_df.arvl_tmstmp[i]) - pd.Timedelta('0 days 00:00:15.000000'))
        ...
        if (END_FLAG == 0):
            if (merge_flag == 1):
                ...
                # STR1= inp_df.arvl_index[i]
                # END1= inp_df.dptr_index[i]
                # out_binry_sig[STR1:END1] = 1

                STR = inp_df.dptr_index[i]
                END = inp_df.arvl_index[i+1]
                out_binry_sig[STR:END] = 0.5

                # STR3 = inp_df.arvl_index[i+1]
                # END3 = inp_df.dptr_index[i+1]
                # out_binry_sig[STR3:END3] = 1
                ...
                # STR4 = inp_df.arvl_index[i]
                # END4 = inp_df.dptr_index[i]
                # out_binry_sig[STR4:END4] = 1 
                i = i + 1  
            #endif
